...
1. Copy the patch for your favorite flavor (`rose-pine.diff`, `rose-pine-moon.diff` or `rose-pine-dawn.diff`) from the `patches` directory into your `st` source directory.
2. Apply the patch by running `patch -F3 -i rose-pine.diff` (replace with your chosen variant).
3. Run `sudo make install` to build and install `st`.
...
```
_Generated by @rose-pine/build@0.9.1_
